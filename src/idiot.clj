...
;--------------------------------------------------| HASH-OBJECT HELPER
(defn hash-object-helper [switch, file-name] (do
                                               (def header-and-blob (str "blob " (count (slurp file-name)) "\000" (slurp file-name)))
                                               (def address (sha1-sum header-and-blob))
                                               (if (= "-w" switch)
                                                 (do (def dir1 (subs address 0 2))
                                                     (def dir2 (subs address 2))
                                                     (.mkdir (File. (str ".git/objects/" dir1)))
                                                     (io/copy (zip-str header-and-blob) (io/file (str ".git/objects/" dir1 "/" dir2)))
                                                     (println address))
                                                 (println address)
                                                 )
                                               ))
...
(defn cat-file [arg]
      (do
        (cond
          ...
          (not (.isDirectory (io/file ".git"))) (println "Error: could not find database. (Did you run `idiot init`?)")
          (and (= 2 (count arg)) (= "-p" (nth arg 0))) (do
                                                         (def blob-location (str ".git/objects/" (subs (nth arg 1) 0 2) "/" (subs (nth arg 1) 2)))
                                                         (if (.isFile (io/file blob-location))
                                                           (do
                                                             (def unzipped-blob (with-open [input (-> blob-location io/file io/input-stream)]
                                                                                 (unzip input)))
                                                           (def start-of-blob (.indexOf unzipped-blob "\000"))
